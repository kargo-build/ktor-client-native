...
/**
 * macOS x64 Native implementation of HybridEngine.
 * 
...
actual class HybridEngine actual constructor(
    ...
    private val cioEngine: HttpClientEngine by lazy {
        Logger.i { "Initializing CIO engine for all requests (macOS x64 Native - HTTPS supported)" }
        CIO.create()
    }
    ...
    actual override suspend fun execute(data: HttpRequestData): HttpResponseData {
        Logger.d { "Using CIO engine for ${data.url} (macOS x64 Native)" }
        return cioEngine.execute(data)
    }

    actual override fun close() {
        Logger.i { "Closing hybrid engine (macOS x64 Native)" }
        cioEngine.close()
        super.close()
    }
}
